...
class MyControlModel(nn.Module):
    ...
    @nn.compact
    def __call__(self, x: Array, embedding: Array) -> dict[str, Array | dict[int, Array]]:
        u = []
        c = nn.Dense(features=self.cond_mlp_outputs, name='linear_1')(embedding[:, :self.cond_mlp_inputs])
        c = ACTIVATIONS[self.activation](c)
        c = nn.Dense(features=self.cond_mlp_outputs, name='linear_2')(c)
        ...
        skip_additions = {i: nn.Conv(
            features=v.shape[-1],
            kernel_size=(1, 1),
            padding="VALID",
            kernel_init=nn.initializers.zeros,
            bias_init=nn.initializers.zeros,
            name=f'controlnet_down_block_{i}',
        )(v) for i, v in enumerate(u)}

        mid_additions = nn.Conv(
            features=self.features[-1],
            kernel_size=(1, 1),
            padding="VALID",
            kernel_init=nn.initializers.zeros,
        ...
        return {
            'skip_additions': skip_additions,
            'mid_additions': mid_additions,
        }
